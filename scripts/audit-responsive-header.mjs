  ...
  for (const profile of profiles) {
    ...
    try {
      ...
      if (EXPECT_LIVE_LEADERBOARD && profile.name === 'mobile-390') {
        ...
        const normalizedLeaderboardText = leaderboardText.toLowerCase();
        ...
        assert(!normalizedLeaderboardText.includes('not connected'), 'live leaderboard modal thinks the production API is unconfigured');
        assert(normalizedLeaderboardText.includes('published competitors'), 'live leaderboard modal did not render the global board');
