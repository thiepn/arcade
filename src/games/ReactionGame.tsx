...
import {
  ...
  getReactionCircuitCompletionBonus,
  getReactionCircuitLabel,
  getReactionCircuitPairIndex,
  getReactionCircuitRoundIndex,
  isReactionCircuitBoundary,
  type ReactionCircuitChoice,
  type ReactionCircuitState,
} from '../lib/reactionCircuits';
...
export const ReactionGame: React.FC<GameComponentProps> = ({
  ...
  const [circuitHud, setCircuitHud] = useState('CALIBRATION CIRCUIT');
  ...
  const getSessionRound = (position: number) => {
    if (position < REACTION_ROUNDS.length) {
      const roundId = getReactionCircuitRoundIndex(position, circuitStateRef.current);
      return REACTION_ROUNDS[roundId];
    }
    return REACTION_OVERTIME_ROUNDS[position - REACTION_ROUNDS.length];
  };
  ...
  const circuitChoicePending = roundIndex < REACTION_ROUNDS.length && mode === 'RESULT' && isReactionCircuitBoundary(roundIndex);

  const publishCircuit = (position = roundIndex) => {
  ...
  const completeAttempt = (
    ...
    if (reactionTimeMs !== null && correct) setHistory((previous) => [...previous, reactionTimeMs]);

    let circuitBonus = 0;
    if (roundIndex >= 2 && roundIndex < REACTION_ROUNDS.length) {
      const activePairIndex = Math.floor((roundIndex - 2) / 2);
      if (!correct) circuitPairCleanRef.current[activePairIndex] = false;
      const endOfPair = roundIndex === 3 || roundIndex === 5 || roundIndex === 7;
      if (endOfPair) {
        const selected = circuitStateRef.current.choices[activePairIndex] ?? 'SPEED';
        circuitBonus = getReactionCircuitCompletionBonus(
          selected,
          activePairIndex,
          circuitPairCleanRef.current[activePairIndex],
        );
      }
    }

    const attemptPoints = scoring.points + circuitBonus;
    const newScore = scoreRef.current + attemptPoints;
    scoreRef.current = newScore;
    setScore(newScore);
    onScoreUpdate(newScore);
    setResult({ reactionTimeMs, correct, reason, points: attemptPoints, grade: scoring.grade });
    setMode('RESULT');

    if (soundEnabledRef.current) {
      if (circuitBonus > 0) sounds.playSuccess();
      else if (!correct) sounds.playHit();
  ...
  const chooseCircuitAndContinue = (choice: ReactionCircuitChoice) => {
    const pairIndex = getReactionCircuitPairIndex(roundIndex);
    if (pairIndex < 0) return;
    circuitStateRef.current = chooseReactionCircuit(circuitStateRef.current, pairIndex, choice);
    publishCircuit(roundIndex + 1);
  ...
  const handleInput = (choice?: ReactionChoice) => {
    ...
    if (mode === 'RESULT') {
      if (circuitChoicePending) {
        if (choice === 'LEFT') chooseCircuitAndContinue('SPEED');
        else if (choice === 'RIGHT') chooseCircuitAndContinue('CONTROL');
        return;
      }
  ...
  useEffect(() => {
    circuitStateRef.current = createReactionCircuitState();
    circuitPairCleanRef.current = [true, true, true];
    setCircuitHud('CALIBRATION CIRCUIT');
    startRound(0);
      ...
      if (mode === 'RESULT') {
        if (circuitChoicePending) {
          if (event.code === 'ArrowLeft' || event.code === 'KeyA' || event.code === 'Digit1') {
            ...
            handleInput('LEFT');
          } else if (event.code === 'ArrowRight' || event.code === 'KeyD' || event.code === 'Digit2') {
            event.preventDefault();
            handleInput('RIGHT');
          }
          return;
  ...
  }, [mode, roundConfig.kind, choiceTarget, roundIndex, circuitChoicePending]);
  ...
  const handlePointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if ((circuitChoicePending || (mode === 'READY' && requiresChoice(roundConfig.kind))) && containerRef.current) {
      const rect = containerRef.current.getBoundingClientRect();
      handleInput(event.clientX < rect.left + rect.width / 2 ? 'LEFT' : 'RIGHT');
      return;
    }
    handleInput();
  };
    ...
    >
      <div className="w-full flex items-start justify-between gap-2 z-10 pointer-events-none">
        <div className="flex flex-col gap-1.5">
          ...
          <span className="px-2.5 py-1 rounded-lg bg-violet-500/10 border border-violet-500/25 text-violet-300 text-[10px] font-mono-arcade w-fit">{roundConfig.label}</span>
          <span data-p23-transform="REACTION CIRCUIT" className="px-2.5 py-1 rounded-lg bg-cyan-500/10 border border-cyan-400/25 text-cyan-200 text-[10px] font-mono-arcade w-fit">REACTION CIRCUIT — {circuitHud}</span>
          {roundIndex >= REACTION_ROUNDS.length && <span className="px-2.5 py-1 rounded-lg bg-amber-500/10 border border-amber-500/30 text-amber-300 text-[10px] font-mono-arcade w-fit">OVERTIME</span>}
        </div>
      ...
      <div className="flex flex-col items-center justify-center gap-5 my-auto z-10 text-center w-full max-w-xl pointer-events-none">
            ...
            <div className="flex items-center gap-2 sm:gap-3 p-3 sm:p-4 rounded-3xl bg-[#18181B]/90 border border-[#27272A] shadow-2xl">
              {[1, 2, 3, 4, 5].map((lightIndex) => (
                <div key={lightIndex} className="p-1.5 sm:p-2 rounded-2xl bg-[#09090B] border border-[#27272A]">
                  <div className={`w-7 h-7 sm:w-9 sm:h-9 rounded-full transition-all duration-75 ${lightIndex <= lightsCount ? 'bg-[#EF4444] shadow-[0_0_20px_#EF4444]' : 'bg-[#27272A]'}`} />
                </div>
              ))}
            </div>
            <div className="flex flex-col items-center gap-2">
              <h2 className="font-mono-arcade font-black text-2xl sm:text-3xl text-white tracking-wide animate-pulse">WAIT FOR THE SIGNAL</h2>
              <p className="max-w-md font-mono-arcade text-xs leading-relaxed text-[#A1A1AA]">{roundConfig.hint}</p>
            </div>
        ...
        {mode === 'DECOY' && (
          <div className="flex flex-col items-center gap-3 animate-in zoom-in-90 duration-75">
            ...
            <h1 className="font-mono-arcade font-black text-5xl sm:text-7xl text-white tracking-widest">HOLD!</h1>
        ...
        {mode === 'READY' && !requiresChoice(roundConfig.kind) && (
          <div className="flex flex-col items-center gap-2 animate-in zoom-in-75 duration-75">
            ...
            <h1 className="font-mono-arcade font-black text-5xl sm:text-7xl text-white tracking-widest drop-shadow-2xl">TAP NOW!</h1>
          </div>
        )}
        ...
        {mode === 'READY' && requiresChoice(roundConfig.kind) && choiceTarget && (
          <div className="w-full flex flex-col items-center gap-5 animate-in zoom-in-75 duration-75">
            ...
            <div className="flex items-center gap-4 sm:gap-7">
              {choiceTarget === 'LEFT' ? <ArrowLeft className="w-20 h-20 sm:w-28 sm:h-28 text-white stroke-[3]" /> : <ArrowRight className="w-20 h-20 sm:w-28 sm:h-28 text-white stroke-[3]" />}
              <h1 className="font-mono-arcade font-black text-4xl sm:text-6xl text-white tracking-widest">{choiceTarget}</h1>
            </div>
            <p className="font-mono-arcade text-xs text-sky-200">TAP THE {choiceTarget} HALF • {choiceTarget === 'LEFT' ? 'A / ←' : 'D / →'}</p>
          </div>
        )}
          ...
          <div className="flex flex-col items-center gap-4 animate-in zoom-in-90 duration-150">
            {result.correct && result.reactionTimeMs !== null ? (
              ...
              <>
                ...
                <h2 className="font-mono-arcade font-black text-3xl sm:text-4xl text-white tracking-wide">{result.reason}</h2>
      ...
      <div className="w-full flex items-end justify-between gap-2 z-10 pointer-events-none min-h-8">
        <div className="flex gap-2">
          {bestTime !== null && <div className="flex items-center gap-1.5 bg-[#18181B]/90 px-2.5 py-1.5 rounded-lg border border-[#27272A] font-mono-arcade text-[10px] text-[#38BDF8]"><Award className="w-3 h-3" /> BEST {bestTime}ms</div>}
          {avgTime !== null && <div className="hidden sm:flex items-center gap-1.5 bg-[#18181B]/90 px-2.5 py-1.5 rounded-lg border border-[#27272A] font-mono-arcade text-[10px] text-[#34D399]"><Gauge className="w-3 h-3" /> AVG {avgTime}ms</div>}
        </div>
        <span className="font-mono-arcade text-[10px] text-[#71717A] text-right">SPACE / TAP • CHOICE: A/D OR ←/→</span>
      </div>
    </div>
  );
};
