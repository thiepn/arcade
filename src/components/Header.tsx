...
import { Volume2, VolumeX, Search, BarChart2, Heart, Clock, Trophy, Sparkles } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-2.5 sm:px-6 h-16 flex items-center justify-between gap-1.5 sm:gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-4 lg:gap-8 min-w-0">
          <button
            type="button"
            id="brand-logo-btn"
            aria-label="Micro Arcade — All Games"
            onClick={() => {
              sounds.playClick();
              onSelectTab('all');
            }}
            className="p19-brand-button flex items-center gap-2 cursor-pointer select-none group shrink-0 rounded-xl bg-transparent text-left"
          >
          ...
          <nav aria-label="Arcade library" className="hidden md:flex items-center gap-1.5 p-1 rounded-xl bg-[#121215] border border-[#27272A] text-xs font-mono-arcade text-[#A1A1AA]">
            <button
              ...
              className={`p19-nav-button px-3 py-1.5 rounded-lg transition-all cursor-pointer font-bold flex items-center gap-1.5 ${
                activeTab === 'all'
            ...
            <button
              ...
              className={`p19-nav-button px-3 py-1.5 rounded-lg transition-all flex items-center gap-1.5 cursor-pointer font-bold ${
                activeTab === 'favorites'
            ...
            >
              <Heart className={`w-3.5 h-3.5 ${activeTab === 'favorites' ? 'fill-current' : ''}`} aria-hidden="true" />
              <span>Favorites</span>
              {favoriteCount > 0 && (
            ...
            <button
              ...
              className={`p19-nav-button px-3 py-1.5 rounded-lg transition-all cursor-pointer font-bold ${
                activeTab === 'recent'
                  ? 'bg-[#27272A] text-white shadow-sm'
                  : 'hover:text-white hover:bg-[#18181B]'
              }`}
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          ...
          <button
            ...
            className="p19-nav-button px-2 sm:px-2.5 py-1.5 rounded-xl bg-gradient-to-r from-cyan-950/40 via-sky-900/30 to-[#121215] border border-cyan-500/40 hover:border-cyan-400 text-cyan-200 text-xs font-mono-arcade font-bold flex items-center gap-1 sm:gap-1.5 transition-all shadow-[0_0_12px_rgba(56,189,248,0.15)] cursor-pointer group shrink-0"
            title="Open Global World Leaderboards & Records"
            aria-label="Open global leaderboards"
          >
            <Trophy className="w-3.5 h-3.5 text-cyan-400 transition-transform" aria-hidden="true" />
            <span className="hidden sm:inline">LEADERBOARD</span>
            <span className="sm:hidden text-[10px]">RANKS</span>
            <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse hidden xs:inline-block" aria-hidden="true" />
          </button>
          ...
          <button
            ...
            className="p19-nav-button px-1.5 sm:px-3 py-1 rounded-xl bg-[#121215] hover:bg-[#1A1A1E] border transition-all cursor-pointer flex items-center gap-1 sm:gap-2 group relative select-none shadow-sm shrink-0 max-w-[110px] sm:max-w-none overflow-hidden"
            style={{
            ...
            aria-label={`Open player profile — ${rankProfile.rankTierName}, level ${levelInfo.level}`}
          >
            {/* Level Orb */}
            <div
            ...
            <div className="hidden md:flex flex-col text-left leading-none min-w-0 overflow-hidden">
              ...
              <div className="w-12 sm:w-16 h-1 bg-[#27272A] rounded-full overflow-hidden mt-0.5 shrink-0" aria-hidden="true">
                <div
          ...
          >
            ...
            <Clock className="w-3 h-3 text-[#71717A] group-hover:text-[#A1A1AA] transition-colors" aria-hidden="true" />
          ...
          <button
            ...
            className={`p19-icon-button p-1.5 sm:p-2 rounded-xl border transition-all cursor-pointer shrink-0 ${
              searchOpen
            ...
            title="Search games (/)"
            aria-label={searchOpen ? 'Close game search' : 'Search games'}
          ...
          >
            {soundEnabled ? (
              <Volume2 className="w-4 h-4 text-[#F43F5E]" aria-hidden="true" />
            ) : (
          ...
          <button
            ...
            className="p19-icon-button p-1.5 sm:p-2 rounded-xl bg-[#121215] hover:bg-[#1F1F23] text-[#A1A1AA] hover:text-white border border-[#27272A] transition-all cursor-pointer hover:border-[#3F3F46] shrink-0"
            title="Arcade Stats, Settings & Achievements"
            aria-label="Open arcade stats and settings"
          >
